...
with st.expander("⬆  Import saved plan"):
    uploaded = st.file_uploader("Drop a .json plan file", type="json", label_visibility="collapsed")
    imported = {}
    if uploaded:
        try:
            imported = json.load(uploaded)
            st.success("Plan imported — fields updated below.")
        except Exception:
...
st.markdown("""
<br><p style="font-size:11px;color:rgba(255,255,255,0.18);text-align:center">
